...
app.get('/', function(req, res) {
	...
	dbo.listCollections().toArray(function(err, collInfos) {
		...
		+ '<div id="showData"></div><br>open orders:'
		+ '<div id="showData2"></div><br>closed orders 24hrs:'
...
function collectionDo(collection){
	collection.find({
                    ...
                    for (var d in doc3) {
						...
						if (doc3[d].trades.bought1 == false){
                        if (parseFloat(doc3[d].trades.lowestAsk) * .998 <= doc3[d].trades.buy1 && parseFloat(doc3[d].trades.lowestAsk) > 0.00000200) {
                        var amount = btc / parseFloat(doc3[d].trades.lowestAsk);
